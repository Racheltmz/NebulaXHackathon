"""Baseline Rail corrugation classifier: frozen TimesFM3 embeddings + trainable
...
each raw [5, 10000] speed+vibration/shock recording, (2) pools channels via a small trainable
...
Model.predict(self, X). X is a list of raw [5, 10000] float arrays; y holds
raw string labels ("Normal"/"Side I"/"Side II") during fit; predict() gets
only X.
"""
...
class _MILPool(nn.Module):
    """Temporal-attention pooling followed by channel-wise gated MIL pooling."""
    ...
    def forward(self, tok: torch.Tensor) -> torch.Tensor:
        # Learn which TimesFM patches contain the discriminative vibration event.
        tw = torch.softmax(self.temporal(tok).squeeze(-1), dim=2).unsqueeze(-1)
        temporal_p = (tok * tw).sum(2)
        max_p = tok.amax(2)
        pooled = torch.cat([temporal_p, max_p], -1)

        # Then learn which sensor channels are reliable for this recording.
        gate = torch.tanh(self.V(temporal_p)) * torch.sigmoid(self.U(temporal_p))
