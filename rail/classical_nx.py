...
def _sparse_proba(Xtr, ytr, Xte):
    """Blend sparse linear selection with a low-weight nonlinear head."""
    lin = make_pipeline(
        StandardScaler(),
        SelectKBest(f_classif, k=min(100, Xtr.shape[1], len(Xtr) - 1)),
        LogisticRegression(
            C=0.03, penalty="l1", solver="liblinear",
            class_weight="balanced", max_iter=3000,
        ),
    )
    nonlin = make_pipeline(
        StandardScaler(),
        SelectKBest(f_classif, k=min(80, Xtr.shape[1], len(Xtr) - 1)),
        SVC(
    ...
    with warnings.catch_warnings():
        ...
        nonlin.fit(Xtr, ytr)
    out = np.zeros((len(Xte), len(CLASSES)))
    for model, weight in ((lin, 0.7), (nonlin, 0.3)):
        p = model.predict_proba(Xte)
        for j, cl in enumerate(model.classes_):
            out[:, list(CLASSES).index(cl)] += weight * p[:, j]
    return out
...
def _base_and_phase(A_tr, y_tr, A_te, phase_kc):
    ...
    base = (
        ...
        + 0.11 * _proba(joint_tr, y_tr, joint_te, 160, 0.01)
        + 0.06 * _retrieve(r_tr, y_tr, r_te, 3)
        + 0.12 * _retrieve(r_tr, y_tr, r_te, 5)
        + 0.13 * _hierarchical(r_tr, y_tr, r_te)
        + 0.11 * _sparse_proba(r_tr, y_tr, r_te)
    )
