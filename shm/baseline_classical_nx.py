...
class Model:
    ...
    def fit(self, X: Sequence[np.ndarray], y: np.ndarray) -> None:
        ...
        self._A, self._y = A, y
        oof = np.zeros(len(y))
        for a, b in KFold(3, shuffle=True, random_state=1701).split(A):
        ...
        self._cal = make_pipeline(StandardScaler(), Ridge(alpha=0.001)).fit(z, residual)
    ...
    def predict(self, X: Sequence[np.ndarray]) -> np.ndarray:
        ...
        corr = np.clip(self._cal.predict(z), -1.5, 1.5)
        return np.maximum(raw * np.exp(0.75 * corr), 1e-6).astype(float)
# EVOLVE-BLOCK-END
