"""Door search-time evaluator. Never loads holdout labels into the candidate;
only ever calls run_candidate_sandboxed(program, X_train, y_train, X_test)
and scores the returned predictions against y_test locally, in this
never-evolved process.
"""
...
INITIAL_PROGRAM = EXPERIMENT_DIR / "baseline_classical.py"
...
def evaluate(program_path: str) -> dict:
    try:
        ...
        cv3 = run_cv(Xtr, ytr, stratified_kfold_indices(ytr, 3), program_path, door_score, SANDBOX_TIMEOUT)
        cv5 = run_cv(Xtr, ytr, stratified_kfold_indices(ytr, 5), program_path, door_score, SANDBOX_TIMEOUT)
