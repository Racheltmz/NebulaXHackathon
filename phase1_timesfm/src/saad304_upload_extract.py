...
def features_batch(model, xs, device, context=512, normalize=True):
...
def main():
    ...
    ap.add_argument("--context", type=int, default=512); ap.add_argument("--batch-size", type=int, default=32); ap.add_argument("--device", default="cuda"); ap.add_argument("--no-normalize", action="store_true")
    ...
    for name, ds in meta.items():
        ...
        for split in ("train", "test"):
            ...
            for start in range(0, len(x), a.batch_size):
                fs.extend(features_batch(model, x[start:start + a.batch_size], device, a.context, not a.no_normalize))
