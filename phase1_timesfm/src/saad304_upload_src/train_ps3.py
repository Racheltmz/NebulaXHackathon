...
def main():
    ...
    if a.task == 'classification':
        ...
        y_mean, y_std = y.mean(), y.std().clamp_min(1e-4); y_scaled=(y-y_mean)/y_std; yv_scaled=(yv-y_mean)/y_std; loss_fn=nn.SmoothL1Loss()
    if a.init:
    ...
    if raw_train is not None:
        ...
        raw_head = nn.Sequential(nn.Conv1d(raw_seq_train.shape[1],32,9,padding=4), nn.GELU(), nn.MaxPool1d(4),
                                 nn.Conv1d(32,64,9,padding=4), nn.GELU(), nn.AdaptiveAvgPool1d(1), nn.Flatten(),
    ...
    for ep in range(a.epochs):
        for batch in loader:
            ...
            if a.task=='regression': pred=pred.unsqueeze(-1)
    ...
    if a.task=='classification':
        ...
        pred=(pred*y_std+y_mean).squeeze(-1)
